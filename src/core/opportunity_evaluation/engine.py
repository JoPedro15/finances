...
and enforcing exposure constraints."""
...
class PortfolioOpportunityEngine:
    ...
    def _resolve_company_exposure(
        ...
        # (e.g. ticker "O" matching every key that contains the letter "o")
        symbol_pattern: re.Pattern[str] = re.compile(
            rf"\b{re.escape(symbol.lower())}\b"
        )
        name_pattern: re.Pattern[str] | None = (
            re.compile(rf"\b{re.escape(asset_name.lower())}\b") if asset_name else None
        )

        for key, exposure in company_exposures.items():
            key_lower: str = key.lower()
            if symbol_pattern.search(key_lower) or (
                name_pattern and name_pattern.search(key_lower)
            ):
                return exposure
    ...
    def rank_assets(
        ...
        for asset in assets_data:
            ...
            symbol_str: str = str(asset["symbol"])
            asset_name_str: str | None = asset.get("name") or asset.get("description")
            ...
            score: AssetScore = strategy.calculate_score(
                ...
                ter=_opt_float(asset.get("ter")),
                trailing_pe=_opt_float(asset.get("trailing_pe")),
                forward_pe=_opt_float(asset.get("forward_pe")),
                low_52w=_opt_float(asset.get("low_52w")),
                high_52w=_opt_float(asset.get("high_52w")),
            )

            # Evaluate sector and country exposure penalty factor
            sector_str: str | None = asset.get("sector") or None
            country_str: str | None = asset.get("country") or None
