"""Unit tests for SQLite schema initialization and integrity constraints in
src/infra/database/schema.py.
"""
...
def test_assets_table_unique_isin_constraint() -> None:
    """Validates ISIN unique constraint on assets table."""
    conn: sqlite3.Connection = get_connection(":memory:")
    initialize_database(conn)
    ...
    cursor.execute(
        "INSERT INTO assets "
        "(isin, name, yahoo_ticker, quantity, average_buy_price, asset_type) "
        "VALUES ('US0378331005', 'Apple', 'AAPL', 1.0, 100.0, 'stock');"
    )
    ...
    with pytest.raises(sqlite3.IntegrityError):
        cursor.execute(
            ...
            "(isin, name, yahoo_ticker, quantity, average_buy_price, asset_type) "
            "VALUES "
            "('US0378331005', 'Apple Duplicate', 'AAPL2', 1.0, 100.0, 'stock');"
        )

    conn.close()


def test_snapshots_table_unique_timestamp_constraint() -> None:
    """Validates timestamp unique constraint on snapshots table."""
    conn: sqlite3.Connection = get_connection(":memory:")
    initialize_database(conn)
    ...
    cursor.execute(
        ...
        "VALUES ('2026-08-19T09:00:00', 1000.0);"
    )

    with pytest.raises(sqlite3.IntegrityError):
        cursor.execute(
            "INSERT INTO snapshots (timestamp, total_value_eur) "
            "VALUES ('2026-08-19T09:00:00', 2000.0);"
        )

    conn.close()


def test_foreign_key_enforcement_invalid_ids() -> None:
    """Validates foreign key enforcement when inserting non-existent foreign keys."""
    conn: sqlite3.Connection = get_connection(":memory:")
    initialize_database(conn)
    ...
    with pytest.raises(sqlite3.IntegrityError):
        cursor.execute(
            ...
            "(snapshot_id, asset_id, native_price, native_currency, value_eur) "
            "VALUES (999, 999, 100.0, 'USD', 90.0);"
        )

    conn.close()


def test_foreign_key_cascade_delete_snapshots() -> None:
...
def test_foreign_key_cascade_delete_assets() -> None:
    """Validates cascade deletion from assets to stock_fundamental_history."""
    conn: sqlite3.Connection = get_connection(":memory:")
    initialize_database(conn)
    ...
    cursor.execute(
        "INSERT INTO assets "
        "(name, yahoo_ticker, quantity, average_buy_price, asset_type) "
        "VALUES ('Tesla', 'TSLA', 10.0, 200.0, 'stock');"
    )
    asset_id: int = cursor.lastrowid or 0

    cursor.execute(
        "INSERT INTO stock_fundamental_history "
        "(asset_id, fetched_at, pe_ratio) VALUES (?, '2026-08-19', 45.0);",
        (asset_id,),
    )
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM stock_fundamental_history WHERE asset_id = ?;",
        (asset_id,),
    )
    row: tuple[int] | None = cursor.fetchone()
    assert (row[0] if row else 0) == 0
