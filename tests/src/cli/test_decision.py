...
def _display_rebalance_results(
    ...
    """Renders strategy coefficients, matrix, and expanded action cards."""
    console.print()

    # Requirement 1: Strategy Coefficient Transparency Panel
    stock_weights: str = (
        f"W_dip: [cyan]{settings.stock_weight_dip:.2f}[/cyan] | "
        f"W_fwd_pe: [cyan]{settings.stock_weight_forward_pe:.2f}[/cyan] | "
        f"W_52w: [cyan]{settings.stock_weight_52w_range:.2f}[/cyan] | "
        f"W_gap: [cyan]{settings.stock_weight_allocation:.2f}[/cyan]"
    )
    etf_weights: str = (
        f"W_dip: [cyan]{settings.etf_weight_dip:.2f}[/cyan] | "
        f"W_ter: [cyan]{settings.etf_weight_ter:.2f}[/cyan] | "
        f"W_gap: [cyan]{settings.etf_weight_allocation:.2f}[/cyan]"
    )
    ...
    # Requirement 2: Table Readability Enhancements
    table: Table = Table(
    ...
    table.add_column("Rank", justify="center", style="cyan", no_wrap=True, min_width=5)
    table.add_column(
        "Symbol", style="bold white", no_wrap=False, overflow="fold", min_width=8
    )
    table.add_column("Type", style="dim", no_wrap=True, min_width=6)
    table.add_column("Price (€)", justify="right", min_width=10)
    table.add_column("Cur %", justify="right", min_width=8)
    table.add_column("Tar %", justify="right", min_width=8)

    if verbose:
        table.add_column(
            "Dip Sc", justify="right", style="dim", no_wrap=True, min_width=8
        )
        table.add_column(
            "Cost Sc", justify="right", style="dim", no_wrap=True, min_width=8
        )
        table.add_column(
            "Gap Sc", justify="right", style="dim", no_wrap=True, min_width=8
        )

    table.add_column("Quant Score", justify="right", style="bold blue", min_width=11)

    if has_ai:
        table.add_column("AI Action", justify="center", min_width=10)
        table.add_column("Urgency", justify="center", min_width=8)
        table.add_column("Conf.", justify="right", min_width=8)
    ...
    # Requirement 3: Enrichment of Individual Asset Action Cards
    ...
    if has_ai and recommendations_map:
        ...
        if active_recs:
            ...
            for symbol, rec in active_recs:
                ...
                # Delta calculation in EUR to reach target allocation
                target_val_eur: float = total_val * (targ_alloc / 100.0)
                current_val_eur: float = total_val * (curr_alloc / 100.0)
                delta_eur: float = target_val_eur - current_val_eur
                delta_str: str = (
                    f"+{delta_eur:,.2f} EUR"
                    if delta_eur >= 0
                    else f"{delta_eur:,.2f} EUR"
                )
                ...
                if target_item.get("asset_type") == "ETF":
                    ...
                    val_ratios = f"P/E (Trailing/Fwd): {tr_str} / {fw_str}"

                score_info: AssetScore | None = score_map.get(symbol)
                breakdown_str: str = ""
                if score_info:
                    breakdown_str = (
                        f"Dip Score: [cyan]{score_info.dip_score:.2f}[/cyan] │ "
                        f"Valuation/Cost Score: "
                        f"[cyan]{score_info.cost_score:.2f}[/cyan] │ "
                        f"Gap Score: "
                        f"[cyan]{score_info.allocation_score:.2f}[/cyan] │ "
                        f"Quant Total: "
                        f"[bold blue]{score_info.total_score:.4f}[/bold blue]"
                    )
                ...
                panel_content: str = (
                    ...
                    f"• [bold]Valuation Metrics:[/bold] {val_ratios}\n"
                    f"• [bold]Factor Scores:[/bold] {breakdown_str}\n"
                    f"{divider}\n"
